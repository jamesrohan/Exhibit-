...
public class EditReservationViewController implements Initializable {
    ...
    private String prefixFromDB;
    private String suffixFromDB;
    @FXML
    ...
    private void handleEditReservation(ActionEvent event) throws IOException 
    {
        if (event.getSource() == editGuest) 
        {
            ...
            if (Crystal_Palace_Management_System.CPMS.UpdateReservation(ReservationNumber,
                    Room_Number, Note, StartDate, EndDate,check_In, check_Out,
                    Balance, cash, Adults, Children, multi_Reservation,
                    Phone_Number, Last_Name, First_Name, Middle_Name,
                    Prefix, Suffix, Nickname, Address, Banned
            )) 
            {
                ...
                controller.subResultLabel.setText("Reservation updated "
                        + "successfully");
            } else 
            {
...
//         GuestPrefix.setValue(rs.getString("Prefix"));
    ...
    private void handleDelete(ActionEvent event) throws IOException 
    {
        if (event.getSource() == Delete) {
            ...
            alert.setHeaderText("");
            alert.setContentText("Are you sure you want to cancel the "
                    + "reservation?");
            ...
            if (result.get() == ButtonType.OK)
            {
                ...
                root = tester.load();
                MainController controller = tester.getController();
                controller.subResultLabel.setText("Reservation Cancelled.");
